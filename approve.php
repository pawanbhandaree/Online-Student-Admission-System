...
if (isset($_GET['admission_id'])) {
    ...
    // Update the admission record's status to 'approved'
    $query = "UPDATE admission SET status = 'approved' WHERE admission_id = ?";
    ...
    if ($stmt->affected_rows > 0) {
        ...
        header("Location: admin_display.php?status=approved");
        exit();
    } else {
